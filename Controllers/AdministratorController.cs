...
namespace KEYREGISTERAUTOMATION.Controllers
{
    ...
    public class AdministratorController : BaseController
    {
        ...
        public AdministratorController(ApplicationDbContext context, IEmailService emailService) : base(context, emailService) { }
        ...
        {
            var floors = await _context.AssignmentRecords
                .AsNoTracking()
                .Where(a => !string.IsNullOrWhiteSpace(a.FloorNumber))
                .Select(a => a.FloorNumber!.Trim())
                .ToListAsync();

            var orderedFloors = floors
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .OrderBy(f => int.TryParse(f, out _) ? 0 : 1)
                .ThenBy(f => int.TryParse(f, out var n) ? n : int.MaxValue)
                .ThenBy(f => f)
                .ToList();
            ...
            return PartialView("_Graphs", vm);
        }

        [HttpGet]
        public async Task<IActionResult> GetTemporaryKeyFrequencyGraphData(
            [FromQuery] List<string>? floors,
            DateTime? startDate,
            DateTime? endDate)
        {
            var selectedFloors = (floors ?? new List<string>())
            ...
            var start = (startDate ?? new DateTime(DateTime.Today.Year, 1, 1)).Date;
            ...
            var rawData = await _context.AssignmentRecords
                .AsNoTracking()
                .Where(a =>
                    a.AssignmentType == "Temporary" &&
                    !string.IsNullOrWhiteSpace(a.FloorNumber) &&
                    selectedFloors.Contains(a.FloorNumber!.Trim()) &&
                    a.AssignedOn >= start &&
                    a.AssignedOn < endExclusive)
                .GroupBy(a => new
                {
                    Year = a.AssignedOn.Year,
                    Month = a.AssignedOn.Month
                })
                .Select(g => new
                {
                    g.Key.Year,
            ...
            while (current <= last)
            {
                var found = rawData.FirstOrDefault(x =>
                    x.Year == current.Year &&
                    x.Month == current.Month);

                var frequency = found?.Frequency ?? 0;
                var monthName = current.ToString("MMM");

                object label;

                if (current == new DateTime(start.Year, start.Month, 1) || current.Month == 1)
                {
                    label = new[] { monthName, current.Year.ToString() };
                }
                else
                {
                    label = monthName;
